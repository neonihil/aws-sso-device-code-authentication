...
def retrieve_aws_accounts(sso_client, aws_sso_token):
    aws_accounts_response = sso_client.list_accounts(accessToken=aws_sso_token, maxResults=100)
...
def retrieve_roles_in_account(sso_client, aws_sso_token, account):
    ...
    roles = [role.get("roleName") for role in roles_response.get("roleList")]
    return sorted(roles)


def retrieve_credentials(sso_client, aws_sso_token, account_id, role_name):
    sts_creds = sso_client.get_role_credentials(accessToken=aws_sso_token, roleName=role_name, accountId=account_id)
...
def print_credentials(credentials, account, role_name):
    print(
        f"Here are your temporary STS credentials for the '{role_name}' role in the AWS account '{account.get('accountName')}' ({account.get('accountId')})"
    )
    print()
...
def show_menu(aws_sso_token, region):
    ...
    human_readable_accounts_list = [f"{account['accountName']} ({account['accountId']})" for account in aws_accounts_list]
    ...
    while back_to_main_menu:
        ...
        if selected_role_index < len(roles_in_account) - 1:
            ...
            selected_role = roles_in_account[selected_role_index]
            credentials = retrieve_credentials(sso_client, aws_sso_token, selected_account.get("accountId"), selected_role)
